...
// Preenche mapa de colisao com moveis, mesas e portas trancadas
function buildBlockedSet(
  ...
  // Tiles estaticas bloqueadas (mesa meeting, arvores, cerca, cachorro)
  for (const tile of STATIC_BLOCKED_TILES) {
    blocked.add(`${tile.x},${tile.y}`)
  }

  // Portas trancadas bloqueiam seus tiles (qualquer room)
  if (lockedDoors) {
    Array.from(lockedDoors).forEach((roomIdx) => {
      const door = ROOM_DOORS[roomIdx]
      if (door) {
